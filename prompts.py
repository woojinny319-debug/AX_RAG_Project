...
3) [금액 표기] 모든 금액은 단위(천원/백만원 등)를 그대로 두지 말고 "원" 단위 전체 숫자로 환산하여 함께 표기하세요. 표의 단위 표기(예: "(단위: 천원)", "(단위: 백만원)")를 반드시 확인해 환산하세요.
   - 예: 표에 "30,683"(단위: 백만원)이면 → "30,683백만원(30,683,000,000원)"으로 적습니다.
   - 예: 표에 "7,039,252"(단위: 천원)이면 → "7,039,252천원(7,039,252,000원)"으로 적습니다.
   - 환산 시 단위를 곱한 값(백만원=×1,000,000, 천원=×1,000)을 콤마 구분으로 정확히 쓰세요.
